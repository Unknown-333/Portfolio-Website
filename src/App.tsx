...
const TECH_ICONS: Record<string, { slug: string; color: string }> = {
  ...
  DuckDB: { slug: 'duckdb', color: 'FFF000' },
  Streamlit: { slug: 'streamlit', color: 'FF4B4B' },
  'Anthropic Claude API': { slug: 'anthropic', color: '191919' },
  SciPy: { slug: 'scipy', color: '8CAAE6' },
  aiohttp: { slug: 'aiohttp', color: '2C5BB4' },
  SQLite: { slug: 'sqlite', color: '003B57' },
  'Telegram API': { slug: 'telegram', color: '26A5E4' },
...
const portfolioData = {
  ...
  projects: [
    ...
    {
      name: "NepalAQI-Ops",
      desc: "Production-grade MLOps pipeline for real-time air quality forecasting in Kathmandu Valley.",
      fullDesc: "End-to-end MLOps system ingesting real-time AQI data from OpenAQ and AQICN, engineering Nepal-specific features (festival calendar, monsoon cycles), training LSTM forecasting models tracked via MLflow, and serving predictions through a FastAPI endpoint with champion/challenger routing. Orchestrated by Airflow, monitored via Prometheus + Grafana, with a Streamlit dashboard and ward-level heatmap.",
      category: "Data Engineering" as const,
      stack: ["Python", "Apache Airflow", "Apache Kafka", "Docker", "MLflow", "FastAPI", "Streamlit", "Prometheus", "Grafana", "DuckDB"],
      impact: "Full MLOps lifecycle — ingestion, feature store, training, serving, monitoring, and drift detection — for a real emerging-market environmental use case.",
      link: "https://github.com/Unknown-333/nepalaqiops",
    },
    {
      name: "Executive Evasion Index (EEI)",
      desc: "NLP pipeline scoring executive evasion on earnings calls to generate a cross-sectional equity trading signal.",
      fullDesc: "Quantitative research system that extracts Q&A pairs from earnings-call transcripts, scores each answer on a Gricean-cooperation rubric (8 evasive tactics) using either a heuristic scorer or Anthropic Claude, then aggregates into cross-sectional factors (EEI_level, EEI_delta, EEI_trend) and back-tests quintile long-short portfolios against SPY. Includes a Streamlit dashboard with 8 pages and PDF/Excel export.",
      category: "Data Science" as const,
      stack: ["Python", "Anthropic Claude API", "Streamlit", "SciPy", "Pandas", "NumPy", "Docker", "GitHub Actions"],
      impact: "Novel alternative-data signal with in-sample Sharpe ≈ 1.46; 74-test suite with mocked LLM; full CI pipeline.",
      link: "https://github.com/Unknown-333/eei_project",
    },
    {
      name: "Sports Betting Stat-Arb Scanner",
      desc: "Market-microstructure engine detecting real-time statistical arbitrage across sportsbook odds.",
      fullDesc: "Production-grade engine treating sportsbooks as fragmented order books — strips Pinnacle vig to derive fair probabilities, scans DraftKings/FanDuel/BetMGM for +EV and cross-book arbitrage opportunities, sizes bets via Kelly criterion, tracks line movements in SQLite, detects steam moves, and runs Monte-Carlo bankroll simulations. 257 tests at 95% coverage.",
      category: "Data Science" as const,
      stack: ["Python", "aiohttp", "NumPy", "SQLite", "Streamlit", "Docker", "Telegram API"],
      impact: "91× vectorized speedup; 257 tests at 95% coverage; real-time scanning with sub-2ms latency.",
      link: "https://github.com/Unknown-333/sports-betting",
    },
  ],
  skills: {
